...
def point_contacts(fig=5, debug=False):
    ...
    if fig == 5: # Two point contacts, on a horizontal line.
        distance = 125.0
        expr = point_contacts_gen([(-distance/2, 0), (distance/2, 0)], radius=pc_radius, J=J)
    if fig == 6: # Three point contacts, arranged on a circle around the center of the mesh.
        ...
        expr = point_contacts_gen(coords, radius=pc_radius, J=J)
    if debug:
...
mesh = from_geofile("film.geo"); unit_length = 1e-9;
Ms = 860e3
...
for figure in [5]:
    ...
    t = 0; dt = 5e-12; pulse_time = 50e-12; t_max = 65e-12;
    while t <= t_max:
