...
_larmor_freq = lambda Ms: 2 * pi * gamma_free * mu0 * Ms
...
print "   w_M= 2 * pi * {:.2g} GHz = {:.2g} Hz.".format(_larmor_freq(Ms_alt)/(2*pi*1e9),_larmor_freq(Ms_alt))
print "2. When computed with Ms= {:.2g} A/m:".format(Ms)
print "   w_M= 2 * pi * {:.2g} GHz = {:.2g} Hz.".format(_larmor_freq(Ms)/(2*pi*1e9),_larmor_freq(Ms))
...
print "   {} time unit = {:.2g} ps.".format(1, 1e12*time(1, _larmor_freq(Ms)))
print "3. When computed with the Larmor frequency for Ms= {:.2g} A/m.".format(Ms_alt)
print "   {} time unit = {:.2g} ps.".format(1, 1e12*time(1, _larmor_freq(Ms_alt)))
