...
| D8 | `Simulation` and `sim_with` default to SciPy rather than legacy Sundials | The native factory defaults to Sundials, but the public simulator default was changed during the port. Both backends now exist; Sundials lifecycle is currently incomplete | Restore Sundials as public default after complete lifecycle validation | **approved conditionally 2026-07-23 (owner meeting)** |
