...
finmag_level = _FINMAG_LOG_LEVELS[
    configuration.get_config_option("logging", "console_logging_level", "DEBUG")]
dolfin_level = _DOLFIN_LOG_LEVELS[
    configuration.get_config_option("logging", "dolfin_logging_level", "WARNING")]
color_scheme = configuration.get_config_option(
    "logging", "color_scheme", "light_bg")

# Parse command line options.
parser = argparse.ArgumentParser(
    description='Parse the logging level and colour scheme.')
...
# Parse only known args so py.test can parse the remaining ones.
...
# to control level separately from dolfin
...
try:
    ...
    raise ValueError("Unkown color scheme: '{}' (allowed values: {})".format(
        color_scheme, ansistrm.level_maps.keys()))

logger.addHandler(ch)

# Now add file handlers for all logfiles listed in the finmag
# configuration file.
for f in logfiles:
    ...
    maxBytes = int(configuration.get_config_option("logging", "maxBytes", 51200))
    backupCount = int(configuration.get_config_option("logging", "backupCount", 1))
