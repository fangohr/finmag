...
def test_hysteresis_loop_and_plotting(tmpdir):
    ...
    sim.hysteresis_loop(H, initial_direction, N, stopping_dmdt=10,
                        save_every=None, save_at_stage_end=False,
                        filename='test_01/no_snapshots.pvd')
    ...
    sim = sim_with(mesh, Ms=1e6, m_init=(0.8, 0.2, 0), alpha=1.0, unit_length=1e-9, A=None, demag_solver=None)#'FK')
    sim.hysteresis_loop(H, initial_direction, N, stopping_dmdt=10,
                        save_every=None, save_at_stage_end=True,
                        filename='test_02/snapshots.pvd')
    ...
    sim = sim_with(mesh, Ms=1e6, m_init=(0.8, 0.2, 0), alpha=1.0, unit_length=1e-9, A=None, demag_solver=None)#'FK')
    sim.hysteresis_loop(H, initial_direction, N, stopping_dmdt=10,
                        save_every=1e-10, save_at_stage_end=False,
                        filename='test_03/snapshots.pvd')
    ...
    (H_vals, m_vals) = sim.hysteresis_loop(H, initial_direction, N, stopping_dmdt=10,
                                           save_every=1e-10, save_at_stage_end=True,
                                           filename='test_04/snapshots.pvd')
    ...
    pvd_files_expected = ['test_04/snapshots.pvd'] + ['test_04/snapshots__stage_{:03d}.pvd'.format(i) for i in xrange(2*N)]
    assert(pvd_files == pvd_files_expected)
