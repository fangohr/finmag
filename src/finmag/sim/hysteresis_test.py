...
# util/plot_helpers_test.py. Must be set before the plot_helpers import.
matplotlib.use("Agg")
...
# Canonical-paths move (2026-07-27): this file now lives in ``src/finmag/sim/``
# (master's ``hysteresis_test.py`` path) instead of ``src/finmag/tests/``, so
# the oracle fixture directory is reached via ``..``/``tests/fixtures``.
_FIXTURE = os.path.join(
    os.path.dirname(__file__), os.pardir, "tests", "fixtures",
    "hysteresis_oracle.json")
