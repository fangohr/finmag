...
def FFT_m(filename, t_step=None, t_ini=None, t_end=None, subtract_values='average'):
    """
    Given a single data file in .ndt format, or a series of data files
    in .npy format (recommended, see below), this methods computes and
    returns the frequencies and the (absolute values of the) Fourier
    transforms of the x, y and z components of the magnetisation m. If
    necessary, the data is first resampled at regularly spaced intervals.
    Note that this performs a real-valued Fourier transform (i.e. it uses
    np.fft.rfft internally) and thus does not return Fourier coefficients
    belonging to negative frequencies.

    If `filename` is the name of a single .ndt file then the Fourier transform
    of the average magneisation is computed. Note that this is *not* recommended
    since it may not detect modes that have certain symmetries which are averaged
    out by this method. A better way is to pass in a series of .npy files,
    which takes the spatially resolved magnetisation into account.
    ...
    *Returns*
    ...
    fft_mx = np.absolute(np.fft.rfft(mx_resampled, axis=0))
    fft_my = np.absolute(np.fft.rfft(my_resampled, axis=0))
    fft_mz = np.absolute(np.fft.rfft(mz_resampled, axis=0))
    n = len(fft_mx)
