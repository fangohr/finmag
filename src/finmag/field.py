...
class Field(object):
    ...
    def get_ordered_numpy_array1(self):
    ...
    def get_ordered_numpy_array2(self):
        """Returns the dolfin function as an ordered numpy array, so that
        in the case of vector fields all components at the same node
        are grouped together."""
        return self.order1_to_order2(self.get_ordered_numpy_array1())
    ...
    def set_with_ordered_numpy_array1(self, ordered_array):
        """Set the field using an ordered numpy array."""
        dtv = df.dof_to_vertex_map(self.functionspace)
        self.set(ordered_array[dtv])

    def set_with_ordered_numpy_array2(self, ordered_array):
        """Set the field using an ordered numpy array."""        
        self.set_with_ordered_numpy_array1(self.order2_to_order1(ordered_array))
