...
This file now sits at the master path ``src/finmag/tests/test_dmi.py``
(formerly ``test_dmi_dolfinx.py``); master's single-function file at that
path (``test_dmi_field``) is fully accounted for below. The OTHER two DMI
ancestors, ``src/finmag/energies/dmi_test.py`` and
``src/finmag/tests/test_dmi_terms.py``, are RETAINED in the tree because
the mapping below leaves functions in each of them uncovered (the PBC pair
and the vestigial ``dmi_term*`` group); they fail/skip in the non-gating
inventory lane as visible backlog.
...
``src/finmag/tests/test_dmi.py`` (1 test function -- NOTE: an earlier
revision of this header swapped the two DMI ancestor filenames; corrected
here against ``git show b5015c5a``, the mapping content itself is unchanged):
...
``src/finmag/energies/dmi_test.py`` (4 test functions; RETAINED in the tree --
the two PBC functions below are deferred, not covered):
...
``src/finmag/tests/test_dmi_terms.py`` (RETAINED in the tree; unconditionally
skipped/dead IN
