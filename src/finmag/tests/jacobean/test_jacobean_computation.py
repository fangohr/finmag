...
def setup_llg_params_near_one(node_count=5, A=3.6 * 4e-7 * np.pi, Ms=6.7e5, K1=4.3, do_precession=True):
    llg = setup_domain_wall_cobalt(
        node_count=node_count, A=A, Ms=Ms, K1=K1, length=1.3, do_precession=do_precession)
    llg.c = 1.23
    llg.gamma = 1.56
    llg.alpha.set(2.35)
    llg.pins = []
    n = llg.m.as_array().size / 3
    # Generate a random (non-normalised) magnetisation vector with norm close
    # to 1
    np.random.seed(1)
...
class JacobeanComputationTests(unittest.TestCase):
    ...
    # Without demag, this scheme should produce an exact result hence set eps
    # to 1

    def compute_jacobean_fd(self, m, eps=1):
        n = self.llg.m.as_array().size / 3
    ...
    def compute_jacobean_jtimes(self, m):
        ...
        for j, v in enumerate(np.eye(3 * n)):
            ...
            self.assertGreaterEqual(
                self.llg.sundials_jtimes(v, jtimes, 0., m, None, tmp), 0)
            jac[:, j] = jtimes
        return jac
