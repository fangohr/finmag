...
(a plain Python ``field_function(t)`` callable) and two preserved legacy
quirks (``t_off=0.0`` is falsy-disabled; ``DiscreteTimeZeeman`` never
advances its update clock *and* never rebuilds its cached energy form after
an interval update, so ``compute_energy()`` silently goes stale while
``compute_field()``/``energy_density()`` stay current -- discovered while
building the oracle fixture below).
...
- coordinate-ordered legacy oracle fixtures (``timezeeman_oracle.json``,
  ...
  update-interval quirk bit-for-bit, including the stale-energy quirk);
...
def test_discrete_time_zeeman_energy_staleness_quirk():
    """Preserved legacy quirk (discovered building the oracle fixture):
    ``update()`` assigns a brand-new ``self.H`` Field directly, bypassing
    ``set_value()``, so the cached energy form ``self.E`` (built once in
    ``setup()``) never sees the update -- ``compute_energy()`` stays at the
    setup-time value forever, even as ``compute_field()`` visibly changes."""
    _, m, Ms = _fields()
    ...
    H_ext = DiscreteTimeZeeman(field_function, dt_update=2)
    H_ext.setup(m, Ms, unit_length=1.0)
    E0 = H_ext.compute_energy()
    H_ext.update(2.0)
    assert _diff(H_ext, [0, 2.0, 0]) < TOL  # field DID update
    assert H_ext.compute_energy() == E0  # energy did NOT
...
def test_time_zeeman_python_scales_cached_spatial_pattern():
    ...
    # must reflect the current (rescaled) field, unlike DiscreteTimeZeeman's
    # preserved staleness quirk above.
