...
1. A MINIMAL-DIFF transcription of the subset of master's
   ...
   - ``test_compute_energy``, ``test_energy_density_function``,
     ...
     are covered in ``test_energies_dolfinx.py`` (bucket-B: aggregate
   ...
   - ``test_compare_stray_field_of_sphere_with_dipolar_field`` --
     intentionally OUT OF SCOPE here: already
     ``@pytest.mark.xfail(reason='dolfin 1.5')`` in master, and it depends
     on ``finmag.example.sphere_inside_airbox``, which under DOLFINx is a
     deferred surface/example that raises ``NotImplementedError`` by name
