...
def compute_belement_magpar(r1, r2, r3):
    res = np.zeros(3)
    compute_belement(np.zeros(3), np.array(r1, dtype=float), np.array(
        r2, dtype=float), np.array(r3, dtype=float), res)
    return res
...
# Solves the demag problem for phi using the GCR method and the native BEM
# matrix
...
class BemComputationTests(unittest.TestCase):
    ...
    def run_symmetry_test(self, formula):
        ...
        for i in xrange(100):
            ...
            v2 = func(
                np.dot(b, r), np.dot(b, r1), np.dot(b, r2), np.dot(b, r3))
