...
``src/finmag/tests/test_fk_demag_dolfinx.py``), so
``git diff b5015c5a..HEAD -- src/finmag/energies/demag/fk_demag_test.py``
...
# at its master path ``src/finmag/energies/demag/fk_demag_test.py`` (formerly
# ``src/finmag/tests/test_fk_demag_dolfinx.py``), two directories up and
# across, hence the two ``os.pardir`` hops.
_FIXTURE = os.path.join(os.path.dirname(__file__), os.pardir, os.pardir,
                        "tests", "fixtures", "fk_demag_oracle.json")
...
def test_deferred_demag_solvers_raise_by_name():
    ...
    # see demag_pbc_test.py, formerly test_treecode_pbc_demag_dolfinx.py) and no
    # longer raises by name.
