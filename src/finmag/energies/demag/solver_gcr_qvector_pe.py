...
class PEQBuilder(object):
    ...
    def get_dof_normal_dict_avg(self, normtionary):
        ...
        avgnormtionary = {
            k: np.array([float(sum(i)) for i in zip(*normtionary[k])]) for k in normtionary}
        # Renormalize the normals
        avgnormtionary = {k: avgnormtionary[
            k] / df.sqrt(np.dot(avgnormtionary[k], avgnormtionary[k].conj())) for k in avgnormtionary}
    ...
    def build_boundary_data(self):
            ...
            if len(cells) == 1:
                ...
                for gdof in globaldoffacet:
                    ...
                    if gdof not in doftonormal:
                        doftonormal[gdof] = []
                    # Prevent redundancy in Normals (for example 3d
                    # UnitCubeMesh CG1)
                ...
                assert 1 == 2, "Expected only two cells per facet and not " + \
                    str(len(cells))
        ...
        self.doflist_double = np.array(
            doftionary.keys(), dtype=self.normtionary[self.normtionary.keys()[0]].dtype.name)
...
##        q = np.zeros(len(self.normtionary))
        ...
        gradphia = df.project(
            df.grad(phia), df.VectorFunctionSpace(self.V.mesh(), "DG", 0))
