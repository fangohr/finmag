...
``src/finmag/energies/cubic_anisotropy_test.py`` (formerly
``src/finmag/tests/test_cubic_anisotropy_dolfinx.py``), so
``git diff b5015c5a..HEAD -- src/finmag/energies/cubic_anisotropy_test.py``
...
# at its master path ``src/finmag/energies/cubic_anisotropy_test.py`` (formerly
# ``src/finmag/tests/test_cubic_anisotropy_dolfinx.py``), one directory up and
# across, hence the ``os.pardir`` hop.
_FIXTURE_DIR = os.path.join(
    os.path.dirname(__file__), os.pardir, "tests", "fixtures")

_FIXTURE = os.path.join(_FIXTURE_DIR, "cubic_anisotropy_oracle.json")
...
def test_spatially_varying_k1_k2_k3_are_supported():
    ...
    divergence is pinned in tests/test_variable_params.py (formerly
    test_variable_params_dolfinx.py). Only legacy string
...
_NATIVE_FIXTURE = os.path.join(
    _FIXTURE_DIR, "cubic_anisotropy_native_oracle.json")
