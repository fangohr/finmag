...
- Task 19 update: `ThinFilmDemag` and `FixedEnergyDW` were the last two
  ...
  returns DOLFINx's own interleaved-per-node dof order (matching every other
  ported interaction's `Field.as_array()` convention) rather than legacy's
  component-major `(3, n).ravel()` layout -- a documented serialisation-only
  deviation; the physical per-(node, component) values are identical.
