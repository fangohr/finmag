...
class MyBoundaryMesh;
void compute_boundary_common(const Mesh& mesh,
                                                  MyBoundaryMesh& boundary,
                                                  bool interior_boundary);


    class MyBoundaryMesh : public dolfin::Mesh
    {
    public:

        MyBoundaryMesh(){}

        MyBoundaryMesh(const Mesh& mesh) { init_exterior_boundary(mesh); }

        /// Initialize exterior boundary of given mesh
        void init_exterior_boundary(const Mesh& mesh) { compute_boundary_common(mesh, *this, false); }

        void init_interior_boundary(const Mesh& mesh) { compute_boundary_common(mesh, *this, true); }

        MeshFunction<unsigned int>& cell_map() { return _cell_map; }

        const MeshFunction<unsigned int>& cell_map() const { return _cell_map; }

        MeshFunction<unsigned int>& vertex_map() { return _vertex_map; }

        const MeshFunction<unsigned int>& vertex_map() const { return _vertex_map; }
