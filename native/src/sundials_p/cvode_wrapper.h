...
#ifndef __FINMAG_ODE_SUNDIALS_CVODE_H2
#define __FINMAG_ODE_SUNDIALS_CVODE_H2
...
namespace finmag { namespace cvode {
    ...
    public:
        ...
        void init(const bp::object &f, double t0, const boost::shared_ptr<dolfin::GenericVector> &vector_ptr);
	...
	*/
    ...
    private:
        ...
        static int rhs_callback(realtype t, N_Vector y, N_Vector ydot, void *user_data) {
            ...
            /*
            bp::object res_obj = bp::call<void>(rhs_func.ptr(),M);
...
//            bp::object fy_arr = nvector_to_array_object(fy);
        ...
        static int spils_jac_times_vec_callback(N_Vector v, N_Vector Jv, realtype t, N_Vector y,
            ...
            ASSERT(false && "spils_jac_times_vec_callback not implemented");
            abort();
    ...
    void Cvode::init(const bp::object &f, double t0, const boost::shared_ptr<dolfin::GenericVector> &vector_ptr) {
        rhs_fn = f;
        const dolfin::GenericVector &v = (*vector_ptr);

        int size = v.size();
        y_n = N_VNew_Serial(size);
        std::vector<double> v_t;
        v.get_local(v_t);
        printf("length=%d \n",size);

        //double *py = (double *) N_VGetArrayPointer(y_n);

        for(int i=0;i<size;i++){
        	//py[i] = v_t[i];
        }

        CHECK_SUNDIALS_RET(CVodeInit, (cvode_mem, rhs_callback, t0, y_n));
