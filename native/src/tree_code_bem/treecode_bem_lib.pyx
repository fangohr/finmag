...
cdef extern from "treecode_bem.h":
...
cdef class FastSum:
    ...
    cdef int num_limit
    cdef double r_eps
    def __cinit__(self,p=3,mac=0.3,num_limit=100,r_eps=0.1):
        self.r_eps=r_eps
    ...
    def init_mesh(self,np.ndarray[double, ndim=2, mode="c"] x_t,
        ...
        init_fastsum(self._c_plan,M,num_faces,self.p,self.mac,self.num_limit,self.r_eps)
        #print 'init sum okay'
    ...
    def directsum(self,np.ndarray[double, ndim=1, mode="c"] phi,np.ndarray[double, ndim=1, mode="c"] u1):
        update_potential_u1(self._c_plan, &u1[0])
        direct_sum(self._c_plan,&phi[0],&u1[0])
