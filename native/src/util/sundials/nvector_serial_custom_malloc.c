/*
    Copy of nvector_serial.c from Sundials patched to use a custom malloc
    function to allocate the data array
 */
#include "nvector_custom_malloc_impl.h"
/*
 * -----------------------------------------------------------------
 * $Revision: 1.1 $
...
N_Vector N_VNew_Serial(long int length)
{
  ...
  if (length > 0) {
    ...
    /* Allocate memory */
    data = NULL;
    data = (realtype *) nvector_custom_data_malloc(length, sizeof(realtype));
