...
A cube with edge length :math:`L`, expressed in units of the intrinsic length scale, :math:`l_{ex}=\sqrt{A/K_m}\,m` is simulated, where :math:`A` is the exchange coupling strength and :math:`K_m=\frac{1}{2}\mu_0 M_S^2\,kg\,m^{-2}\,s^{-2}` the magnetostatic energy density.
